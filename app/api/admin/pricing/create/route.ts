...
const bodySchema = z.object({
  ...
  extra_adult_pricing_type: z.enum(["fixed", "not_allowed"]).optional(),
...
export async function POST(request: Request) {
  ...
  const [row] = await db
    ...
    .values({
      ...
      childPricingType: parsed.data.child_pricing_type ?? "fixed",
      pricingMode: parsed.data.pricing_mode ?? "per_person",
      extraAdultPricingType: parsed.data.extra_adult_pricing_type ?? "fixed",
